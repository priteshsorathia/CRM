...
import { restaurantModuleMap, restaurantModules } from "@/data/restaurantModules";

export function generateStaticParams() {
  return restaurantModules.map((module) => ({ slug: module.slug }));
}

export default async function RestaurantModuleDetailPage({ params }) {
  const { slug } = await params;
  const module = restaurantModuleMap[slug];
    ...
    <div className="bg-white">
      ...
      <section className="border-b border-[#edf0f5] bg-[linear-gradient(180deg,#f8faff_0%,#ffffff_100%)]">
        <div className="mx-auto max-w-[1240px] px-4 py-16 sm:px-6 lg:px-10 lg:py-14">
          <Link
            href="/"
            className="mb-8 inline-flex items-center gap-2 text-sm font-semibold text-[#2563eb]"
          >
          ...
          <div className="grid items-center gap-12 lg:grid-cols-[1.15fr_0.85fr]">
            <div>
              <div className="inline-flex items-center gap-2 rounded-full bg-[#eff6ff] px-5 py-2 text-sm font-semibold text-[#2563eb]">
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                {module.features.map((feature) => (
                  <span
                    key={feature}
                    className="rounded-full border border-[#dbe3ff] bg-white px-4 py-2 text-sm font-medium text-[#2563eb]"
                  >
                    {feature}
                  </span>
                ))}
              ...
              <div className="mt-10 flex flex-col gap-4 sm:flex-row">
                <Link
                  ...
                  className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#2563eb] px-6 py-3 text-base font-semibold text-white transition hover:bg-[#1d4ed8]"
                >
                  Start Free Trial
            ...
            <div className="rounded-[28px] border border-[#edf0f5] bg-white p-8 shadow-[0_20px_60px_rgba(15,23,42,0.06)]">
              <div
                className={`mb-6 flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br ${module.iconClassName} shadow-[0_15px_35px_rgba(37,99,235,0.22)]`}
              >
              ...
              <div className="mt-6 space-y-4">
                {section.items.map((item) => (
                  <div key={item} className="flex gap-3">
                    <CheckCircle2 className="mt-1 h-5 w-5 flex-shrink-0 text-[#2563eb]" />
        ...
        <div className="mx-auto max-w-[1240px] px-4 sm:px-6 lg:px-10">
          ...
          <div className="mt-10 grid gap-6 md:grid-cols-2 xl:grid-cols-4">
            {module.workflow.map((step, index) => (
              ...
              >
                <div className="flex h-11 w-11 items-center justify-center rounded-full bg-[#eff6ff] text-lg font-bold text-[#2563eb]">
                  {index + 1}
                </div>
